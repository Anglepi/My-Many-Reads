...
\textit{Aspell} es un corrector ortográfico \textit{Open Source} que empleo para la corrección de mi documentación. El principal motivo de la elección de esta es que, a parte de ser \textit{Open Source}, tiene soporte para ficheros de LaTeX. En el repositorio hay \href{https://github.com/Anglepi/My-Many-Reads/blob/main/scripts/spellcheck.sh}{un \textit{script}} que se encarga de su uso, apoyándose en un diccionario personalizado de \href{https://github.com/Anglepi/My-Many-Reads/blob/main/scripts/spellcheckDictionary.txt}{excepciones} ya que esta comprobación ortográfica es parte del proceso de integración, y si no supera esta comprobación no se permitirá la inclusión de los cambios a la rama principal.

Más adelante se explicará su funcionamiento y uso en la sección referente al proceso de integración continua implementado.
...
Al principio de su \href{https://github.com/Anglepi/My-Many-Reads/blob/main/.github/workflows/spellcheck.yml}{fichero de configuración} se define el nombre del \textit{workflow} y los momentos en los que se ejecuta. En este caso, se ejecuta tras cada \textit{push} que afecte a algún fichero relacionado con la documentación, es decir, de extension \textit{.tex} o \textit{.md}, que afecte a la definición propio \textit{workflow} o al \textit{script} encargado de la corrección ortográfica.
...
Este \textit{job} en concreto se ejecutará en la última versión estable de Ubuntu, el propio workflow se encarga de inicializar este entorno de ejecución, sobre el cual se ejecutarán una serie de pasos que tienen como objetivo realizar la revisión ortográfica.
...
\begin{enumerate}
    ...
    \item Tras esto, se ejecuta el comando que instala \textit{aspell} en el entorno, junto con sus complementos para soporte de los idiomas inglés y español.
...
En este caso, solo se define un array de versiones de \textit{Python} soportadas, por lo que los diferentes steps se ejecutarán para cada una de estas versiones definidas:
...
\begin{enumerate}
    ...
    \item Se genera la documentación usando el \textit{target} definido en el \href{https://github.com/Anglepi/My-Many-Reads/blob/main/Makefile}{\textit{Makefile}}, sin necesidad de instalaciones previas gracias a la imagen empleada.
...
\section{Representación y almacenamiento de datos}
...
\subsection{Clases que representan las entidades}
...
\subsection{Importando datos reales}

Existen muchos conjuntos de datos disponibles en internet, de forma gratuita, sobre casi cualquier temática. El caso de los libros no es una excepción, por lo que podemos aprovechar esto para introducir una carga inicial de datos reales para continuar aportando valor a la plataforma.

A la hora de buscar un conjunto de datos para importar hay que tener en cuenta, además de factores de legalidad y licencias de uso, qué información incluyen, puesto que tratar con información sobrante no es problema, pero si falta información, las necesidades no se cubren.

Explorando las diferentes opciones, se encontró con \href{https://zenodo.org/record/4265096}{este conjunto de datos} que incluye la inmensa mayoría de información necesaria, salvo algunos valores de los que de momento se puede prescindir, como la fecha de nacimiento de los autores.

Para importar toda esta información se ha desarrollado un \href{https://github.com/Anglepi/My-Many-Reads/blob/main/scripts/import_data.py}{\textit{script}} que procesa cada línea del fichero \textit{CSV} obtenido anteriormente y lo importa  en la base de datos. Este \textit{script} está preparado para funcionar con una base de datos vacía, si se ejecutase varias veces, duplicaría la información, ya que no comprueba si esta ya se encontraba en la base de datos.

En primer lugar se van introduciendo los autores, para lo cual se almacena en una variable los ya introducidos junto con su identificador, de forma que si para un siguiente libro se repite un autor, se salta su inserción ya que se encuentra en dicha variable. Para los géneros, ocurre una lógica similar.

Después se introducen los libros, y como este conjunto de datos está orientado a los estos, no deberían existir entradas repetidas, por lo que no es necesario llevar un control de las obras ya insertadas. Tras la inserción de cada libro, se introducen a su vez las relaciones de este con sus autores y géneros, aprovechando que las consultas de inserción están diseñadas para devolver los nuevos identificadores, con los que se construyen estas relaciones.

Como suele ser común en este tipo de procesos, los datos no son perfectos ni tienen por qué encajar con los formatos deseados. Por ello, en este caso, se realizan operaciones de preprocesamiento tales como:

\begin{itemize}
    \item Eliminar las comillas simples en los campos de texto, ya que rompen las consultas.
    \item En el caso de campos con valores requeridos, pero no disponibles en el conjunto de datos, aplicar un valor por defecto. Esto ocurre para la fecha de nacimiento del autor.
    \item La fecha de publicación viene en un formato diferente, y a veces vacía. Hay que transformar este formato al especificado en la base de datos e introducir un valor por defecto para lidiar con valores vacíos.
\end{itemize}

Dado que el \textit{commit} de estas transacciones solo se realiza al finalizar la lectura del fichero CSV, esta importación se trata de un proceso ``todo o nada'', ya que si ocurre algún error, ninguno de los datos serán introducidos en la base de datos.

\subsection{Gestor de datos}

La primera implementación de la API se introdujo en el segundo PMV (ver \underline{\nameref{pmv2}}). En este momento, al ser una fase temprana del proyecto, se carecía de una base de datos, por lo que para poner este código a prueba se introdujeron datos de prueba y lógica en la propia API, aunque esto incumpla el principio de responsabilidad única.
...
Fue a continuación, en el tercer PMV (ver \underline{\nameref{pmv3}}) cuando se trabajó en solventar esta situación. Se contemplaban dos posibilidades, una de ellas siendo una implementación tradicional consistente en una conexión a una base de datos y procesamiento de consultas, y otra de más alto nivel, usando algún ORM (\textit{Object-Relational Mapping}).
...
Este tipo de herramientas exige un pequeño esfuerzo para su aprendizaje y adicionalmente, no se puede saber qué operaciones realizan internamente para gestionar las consultas, lo que da pie a que estas sean más ineficientes. Dado que representan una capa intermedia entre el código y la base de datos, el control sobre esta también se ve limitado en cierta medida.
...
Este sistema es uno de los puntos más importantes del proyecto, pues es el que más atractivo aporta con respecto a los usuarios. Se compone, básicamente, de dos subsistemas: recomendaciones automáticas basadas en bibliotecas y recomendaciones creadas por usuarios.

\subsection{Recomendaciones de los usuarios}

Para cada pareja de libros, los usuarios podrán escribir un comentario para justificar la recomendación entre ambos. Otros usuarios podrán votar dichos comentarios con el fin de apoyar esa recomendación. Teniendo esto en cuenta, se pueden sacar algunos razonamientos al respecto, que ayudarán a guiar la implementación:
...
Gracias al apoyo de este análisis se puede definir una estructura de datos apropiada para representar estas recomendaciones de forma óptima. Existirá una única instancia de relación entre dos libros, ajena al orden, a la que referenciarán todos los comentarios de los usuarios que apoyen dicha recomendación.


\subsection{Recomendaciones automáticas}

Para elaborar un algoritmo que pueda hacer sugerencias de manera automática para los usuarios, es necesario en primer lugar ser capaz de determinar los gustos de dicho usuario. Teniendo en cuenta que un usuario puede organizar sus lecturas en varias bibliotecas, se van a generar las recomendaciones en base a los gustos definidos en una biblioteca específica, aportando mayor versatilidad a los usuarios de cara a la búsqueda de nuevas lecturas.

El primer paso a la hora de trabajar en este algoritmo es pensar en qué es lo que hace que un libro sea una buena recomendación para una persona concreta. Lo primero que se viene a la mente al pensar en ello es conocer los gustos de esa persona, y en este caso, los gustos vienen definidos por la biblioteca que el usuario ha escogido para generar las recomendaciones.

Una vez conocidos sus gustos, habrá que explorar una colección de libros que no incluyan los que el usuario ya ha leído, y estudiar como encaja cada uno de estos nuevos libros con los de la biblioteca dada por el usuario. Cuanto más similar sea un libro candidato con los gustos definidos por el usuario, más probable es que se trate de una buena recomendación.

Ahora que este problema y su solución están definidos en lenguaje natural, se puede empezar a aproximar una solución más técnica y detallada, dividiendo este proceso en partes tratables y coherentes:

\begin{itemize}
    \item Primero, habrá que construir un perfil de gustos basado en una biblioteca concreta.
    \item Después, se recorrerá una lista de libros candidatos, entre los cuales no debería haber ninguno leído por el usuario, y de alguna forma se calculará su nivel de compatibilidad con este perfil de gustos.
    \item Una vez obtenida la compatibilidad, se ordenará la lista de libros candidatos de más a menos compatible, formando así la lista de recomendaciones. Este es un paso muy sencillo que no requiere más explicación.
\end{itemize}

\subsubsection{Construyendo el perfil de gustos}

Como ya se ha comentado antes, este perfil estará basado en una biblioteca, por lo que para construirlo habrá que volver a echar un vistazo a la información que contiene una biblioteca, que en este caso claramente la única información de interés que ayuda a definir el perfil son sus entradas. Volviendo al \hyperref[dbdiagram]{\underline{diagrama de la base de datos}} se aprecia que una entrada de biblioteca está compuesta por un libro (más bien su ID, suficiente para obtener la información de este), una puntuación asignada por el usuario a dicho libro, y un estado.

Bien, esto es suficiente para conocer las preferencias, ya que indican que libros se han leído y cuanto han gustado cada uno de ellos, por lo que ahora se puede idear alguna forma de construir un perfil basado en esta información. Para ello, vamos a ver cada una de las propiedades de entre toda la información disponible para estudiar el posible uso e importancia de cada una de estas.

El \textit{\textbf{score}} es una de estas, y representa cuanto le ha gustado al usuario dueño de la biblioteca el libro asociado a esa entrada. Este valor nos puede dar una idea de qué libros gustan más y cuales gustan menos, de forma que podemos darle más importancia a las características de un libro con mayor nota que a las de uno con menos nota.

El \textit{\textbf{status}} también sirve de ayuda, ya que dependiendo del estado, un libro puede ser más significativo para definir los gustos que otro. Los posibles estados son: \textit{PLAN\_TO\_READ, CURRENTLY\_READING, COMPLETED, DROPPED} y \textit{ON\_HOLD}, además de que se permite no establecer ningún valor a esta propiedad. Como claramente se deduce, es más probable que un libro con estado \textit{COMPLETED}, es decir, que se ha leído al completo, represente más los gustos de un usuario que uno cuyo estado sea \textit{DROPPED}, el cual indica que lo empezó a leer pero por algún motivo acabó dejándolo. Por otra parte, las entradas \textit{PLAN\_TO\_READ}, como no se consideran leídas, no se tendrán en cuenta.

Por último, tenemos el \textbf{libro} en cuestión. Las propiedades que más caracterizan a un libro son su \textbf{autor o autores}, pues son los que aportan un estilo narrativo además de la fuente de las ideas plasmadas en sus obras, los \textbf{géneros}, que definen la temática y la \textbf{sinopsis} que es una breve introducción a los temas tratados en el libro.

Teniendo ya estudiadas las características que definen un libro junto a las que indican cuanto ha gustado dicho libro, podemos trabajar en un algoritmo que resuma y agrupe toda esta información, que resultará en el requerido perfil de gustos, un elemento fundamental a la hora de poder obtener las recomendaciones. Este perfil de gustos es representado por la clase \href{https://github.com/Anglepi/My-Many-Reads/blob/main/mmr-backend/library_stats.py}{\textit{LibraryStats}} implementada en \textit{My Many Reads}.

El proceso a seguir para construir el perfil dadas todas estas condiciones es bastante similar al de operaciones tipo \textit{reduce}\cite{reduce}. Se va iterando sobre las entradas de la biblioteca y para cada \textbf{autor} y \textbf{género} encontrado, se va acumulando una puntuación calculada para ellos en base al \textbf{\textit{score}} y el \textbf{\textit{status}}, teniendo en cuenta que cada una de estas propiedades tiene un peso, es decir, un valor numérico que representa su importancia, y que es susceptible a ser modificado para ajustar cuanto se valora cada una de estas propiedades, afectando así al resultado final, la lista de recomendaciones.

Para valorar la \textbf{sinopsis}, la situación gana un gran nivel de complejidad, puesto que su estudio requiere de técnicas más avanzadas que implican un mayor coste y que por tanto se aplazó a fases posteriores del desarrollo que aún no se han alcanzado. Esto no significa que no se haya estudiado cómo hacerlo, sino que los costes de implementarlo son altos en proporción al beneficio que aportan, en comparación con otros requisitos del proyecto, y por tanto no se encontraba entre las prioridades más inmediatas.

Existen dos posibles enfoques a tener en cuenta para extraer información de una sinopsis que sirva para incrementar el valor del perfil de gustos. El primero consiste usar algún algoritmo para extraer las palabras clave, realizar un conteo de estas y valorarlas de manera similar a como se hace con los \textbf{autores} y \textbf{géneros}. El proceso de extracción de palabras clave implica el uso de algoritmos de \textit{Machine Learning} y lo más común es que se empleen redes neuronales. Plataformas como \href{https://monkeylearn.com/}{MonkeyLearn} con su \href{https://monkeylearn.com/api/v3/#extractor-api}{Extractor API} y el gigante Amazon, a través de \href{https://aws.amazon.com/es/comprehend/}{Amazon Comprehend}, ofrecen los servicios, con un precio, para realizar este análisis sobre textos, e incluso pueden llegar a aportar detalles adicionales sobre las palabras clave extraídas.

La otra opción a tener en cuenta, aún más compleja que la anterior, es emplear algoritmos de LSA, \textit{Latent Semantic Analysis}\cite{lsa}, que analizan los textos y permiten extraer diferentes temáticas detectadas definidas por grupos de palabras. Esto nos permitiría detectar las temáticas más comunes entre los libros de una biblioteca, y valorarlas, de nuevo, por \textbf{\textit{score}} y \textbf{\textit{status}}, que son las dos propiedades que indican cuanto gusta un libro.

\subsubsection{Comparando los candidatos con el perfil de gustos}

Anteriormente se ha definido como crear un perfil de gustos, de modo que para una librería, se dispone de un conjunto de valores que puntúan la aparición de determinadas características en los libros a comparar. Esto hace posible calcular una puntuación para cada candidato a recomendación, que será mayor cuanto más encaje dicho candidato con los gustos definidos en el perfil.

Partiendo de un conjunto de libros, los posibles candidatos, se itera sobre ellos y para cada uno se calcula su similitud: partiendo de un valor inicial de 0, si el candidato posee alguno de los autores definidos en el perfil de gustos, se suma el valor de la importancia de dicho autor a su puntuación. Esta estrategia se sigue con el resto de características recogidas en el perfil de gustos, resultando en un valor que representa el nivel de similitud del candidato con el perfil de gustos, que hace las veces de puntuación.

Una vez calculada la puntuación de cada candidato, basta con ordenar esta lista a través de este valor de mayor a menor, y ya se obtiene la lista de recomendaciones automáticas. Esto, por supuesto, se trata de una primera versión de un algoritmo que puede ganar un nivel de complejidad muy alto para mejorar su precisión, en una sección posterior se discuten varias posibles \hyperref[mejoras en las recomendaciones]{\underline{mejoras a este sistema}}, además del posible estudio de la sinopsis mencionado anteriormente.