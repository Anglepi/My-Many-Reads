...
El \textit{\textbf{status}} también sirve de ayuda, ya que dependiendo del estado, un libro puede ser más significativo para definir los gustos que otro. Los posibles estados son: \textit{PLAN\_TO\_READ, CURRENTLY\_READING, COMPLETED, DROPPED, ON\_HOLD} además de que se permite no establecer ningún valor a esta propiedad. Como claramente se deduce, es más probable que un libro con estado \textit{COMPLETED}, es decir, que se ha leído al completo, represente más los gustos de un usuario que uno cuyo estado sea \textit{DROPPED}, el cual empezó a leer pero por algún motivo acabó dejándolo. Por otra parte, las entradas \textit{PLAN\_TO\_READ}, como no se consideran leídas, no se tendrán en cuenta.
