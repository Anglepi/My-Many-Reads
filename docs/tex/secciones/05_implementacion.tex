...
\begin{itemize}
    ...
    \begin{itemize}
        \item \href{https://github.com/Anglepi/My-Many-Reads/issues/66}{\textbf{HU-5.1}} Encontrar una estructura de datos que permita comparar una biblioteca con un libro.
        \item \href{https://github.com/Anglepi/My-Many-Reads/issues/67}{\textbf{HU-5.2}} Hallar la forma de medir la compatibilidad entre un libro y las estadísticas recopiladas de una biblioteca.
        \item \href{https://github.com/Anglepi/My-Many-Reads/issues/68}{\textbf{HU-5.3}} Crear una lista de recomendaciones a partir de una librería.
    \end{itemize}
\end{itemize}
...
Al momento de crear una recomendación se indica el comentario a incluir y el usuario autor. Dicho comentario tendrá una puntuación inicial igual a cero, que incrementará según otros usuarios lo voten. Si la pareja de libros no tenía ninguna recomendación previa, se creará con el comentario concreto, y si existía, simplemente se añadirá dicho comentario.

Cada recomendación, por tanto, queda compuesta de una pareja de libros identificados por su \textit{ISBN}. Con el fin de encontrar recomendaciones concretas con mayor facilidad, esta pareja se ordena al momento de almacenarse.

Adicionalmente, se almacena una lista de comentarios. Cada uno de estos comentarios se identifica por su autor, contiene el texto indicado y una puntuación.

Mediante algo de lógica se controla fácilmente la creación de recomendaciones, adición de comentarios a recomendaciones existentes y votaciones, los cuales se pueden ver fácilmente en \href{https://github.com/Anglepi/My-Many-Reads/blob/main/mmr-backend/user_recommendation.py}{este fichero} y en \href{https://github.com/Anglepi/My-Many-Reads/blob/main/mmr-backend/api.py}{la api}.
...
Este algoritmo de recomendaciones ofrece como resultado una lista ordenada de libros ordenados de mayor a menor relevancia con respecto a la biblioteca especificada. Para construir esta lista se sigue un proceso que puede dividirse en tres pasos principales:

\begin{enumerate}
    \item Se generan unas estadísticas generales de la biblioteca, agrupando toda la información proveniente de los libros que la componen que se considere relevante para determinar los gustos. Las dos propiedades más evidentes son los géneros y los autores, por lo que se pueden agrupar una lista de cada uno de ellos junto con el número de apariciones de éstos en la biblioteca, ponderados según la puntuación otorgada por el usuario al libro.
    \item Mediante una función capaz de comparar un libro con las estadísticas recogidas, se otorga una puntuación de compatibilidad del libro con respecto a una biblioteca. Esta función comparará las propiedades del libro con las de las estadísticas recogidas, otorgando un peso para ajustar la importancia de cada una de ellas permitiendo así generar un número que haga las veces de puntuación.
    \item Finalmente, con las estadísticas de una biblioteca recogidas y una función para puntuar libros respecto a estas se puede generar la lista de recomendaciones. Para ello, recorremos la lista de libros candidatos, de los cuales debemos eliminar los que ya se encuentren en la biblioteca, y puntuamos cada uno de ellos. Estos libros se ordenan por puntuación, facilitando así devolver un subconjunto de los más relevantes.
\end{enumerate}
