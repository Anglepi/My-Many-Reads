...
Explorando las diferentes opciones, se encontró con \href{https://zenodo.org/record/4265096}{este conjunto de datos} que incluye la inmensa mayoría de información necesaria, por no decir toda, salvo algunos valores de los que de momento se puede prescindir, como la fecha de nacimiento de los autores.
...
Después se introducen los libros, y como este conjunto de datos está orientado a los libros, no deberían existir libros repetidos, por lo que no es necesario llevar un control de los ya insertados. Tras la inserción de cada libro, se introducen a su vez las relaciones de este con sus autores y géneros, aprovechando que los procesos de inserción devuelven los identificadores con los que se construyen estas relaciones.

Como suele ser común en este tipo de procesos, los datos no son perfectos ni tienen por qué encajar con los formatos deseados. Por ello, en este caso, se realizan operaciones de preprocesamiento de datos tales como:

\begin{itemize}
    \item Eliminar las comillas simples en los campos de texto
...
Dado que el \textit{commit} de estas transacciones solo se realiza al finalizar la lectura del fichero CSV, esta importación se trata de un proceso ``todo o nada'', donde si algún error ocurre, ninguno de los datos serán introducidos en la base de datos.
